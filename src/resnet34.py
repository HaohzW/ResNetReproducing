...
class ResNet34(nn.Module):
    def __init__(self, class_number=10):
        ...
        self.downsample23 = nn.Sequential(
        ...
        self.downsample34 = nn.Sequential(
        ...
        self.downsample45 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x += x_shortcut
        x = F.relu(x)
        x_shortcut = self.downsample23(x)
        #conv3_x
        x = self.conv3_1(x)
        x += x_shortcut
        x = F.relu(x)
        ...
        x += x_shortcut
        x = F.relu(x)
        x_shortcut = self.downsample34(x)
        # conv4_x
        x = self.conv4_1(x)
        x += x_shortcut
        x = F.relu(x)
        ...
        x += x_shortcut
        x = F.relu(x)
        x_shortcut = self.downsample45(x)
        # conv5_x
        x = self.conv5_1(x)
        x += x_shortcut
        x = F.relu(x)
